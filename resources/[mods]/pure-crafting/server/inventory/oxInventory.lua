...
function checkItem(source, item, amount)
    local oxAmount = exports.qb-inventory:Search(source, 'count', item)
    if oxAmount >= amount then return true end
    return
end

function getItems(source, item)
    local amt = exports.qb-inventory:Search(source, 'count', item)
...
function giveItem(source, item, amount)
    local canCarry = exports.qb-inventory:CanCarryItem(source, 'count', item)
    if not canCarry then return end
    local success, response = exports.qb-inventory:AddItem(source, item, amount)
    return success
end
