...
local function UpdateBlips()
    ...
    for i = 1, #players do
        local v = players[i]
        if v and (v.PlayerData.job.type == 'police' or v.PlayerData.job.type == 'ems') and v.PlayerData.job.onduty then
...
QBCore.Commands.Add('takedna', Lang:t('commands.takedna'), { { name = 'id', help = Lang:t('info.player_id') } }, true, function(source, args)
    ...
    if not OtherPlayer or Player.PlayerData.job.type ~= 'police' or not Player.PlayerData.job.onduty then return end
    if exports.ox_inventory:RemoveItem(src, 'empty_evidence_bag', 1, false, 'qb-policejob:takedna') then
        local info = {
            label = Lang:t('info.dna_sample'),
            type = 'dna',
            dnalabel = DnaHash(OtherPlayer.PlayerData.citizenid)
        }
        if not exports.ox_inventory:AddItem(src, 'filled_evidence_bag', 1, false, info, 'qb-policejob:takedna') then return end
        TriggerClientEvent('ox_inventory:client:ItemBox', src, QBCore.Shared.Items['filled_evidence_bag'], 'add')
    else
...
QBCore.Functions.CreateUseableItem('moneybag', function(source, item)
    ...
    if not Player.PlayerData.job.type == 'police' then return end
    if not exports.ox_inventory:RemoveItem(src, 'moneybag', 1, item.slot, 'qb-policejob:moneybag') then return end
...
RegisterNetEvent('qb-policejob:server:stash', function()
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if Player.PlayerData.job.type ~= 'police' then return end
...
RegisterNetEvent('qb-policejob:server:trash', function()
    ...
    if not Player then return end
    if Player.PlayerData.job.type ~= 'police' then return end
    exports.ox_inventory:OpenInventory(src, 'policetrash', {
        maxweight = 4000000,
        slots = 300,
    })
end)
...
RegisterNetEvent('qb-policejob:server:evidence', function(currentEvidence)
    ...
    if not Player then return end
    if Player.PlayerData.job.type ~= 'police' then return end
    exports.ox_inventory:OpenInventory(src, currentEvidence, {
        maxweight = 4000000,
        slots = 500,
    })
end)
...
RegisterNetEvent('police:server:SearchPlayer', function()
    ...
    if PlayerData.job.type ~= 'police' then return end
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if not SearchedPlayer then return end
        exports.ox_inventory:OpenInventoryById(src, tonumber(player))
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName('handcuffs') and Player.PlayerData.job.type ~= 'police') then return end
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if (Player.PlayerData.job.type == 'police' or Player.PlayerData.job.name == 'ambulance') or (EscortPlayer.PlayerData.metadata['ishandcuffed'] or EscortPlayer.PlayerData.metadata['isdead'] or EscortPlayer.PlayerData.metadata['inlaststand']) then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.type ~= 'police' then return end
...
RegisterNetEvent('police:server:SeizeCash', function(playerId)
    ...
    exports.ox_inventory:AddItem(src, 'moneybag', 1, false, info, 'police:server:SeizeCash')
    TriggerClientEvent('ox_inventory:client:ItemBox', src, QBCore.Shared.Items['moneybag'], 'add')
...
RegisterNetEvent('police:server:RobPlayer', function(playerId)
    ...
    exports.ox_inventory:OpenInventoryById(src, playerId)
...
RegisterNetEvent('evidence:server:AddBlooddropToInventory', function(bloodId, bloodInfo)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if exports.ox_inventory:RemoveItem(src, 'empty_evidence_bag', 1, false, 'evidence:server:AddBlooddropToInventory') then
        if exports.ox_inventory:AddItem(src, 'filled_evidence_bag', 1, false, bloodInfo, 'evidence:server:AddBlooddropToInventory') then
            TriggerClientEvent('ox_inventory:client:ItemBox', src, QBCore.Shared.Items['filled_evidence_bag'], 'add')
...
RegisterNetEvent('evidence:server:AddFingerprintToInventory', function(fingerId, fingerInfo)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if exports.ox_inventory:RemoveItem(src, 'empty_evidence_bag', 1, false, 'evidence:server:AddFingerprintToInventory') then
        if exports.ox_inventory:AddItem(src, 'filled_evidence_bag', 1, false, fingerInfo, 'evidence:server:AddFingerprintToInventory') then
            TriggerClientEvent('ox_inventory:client:ItemBox', src, QBCore.Shared.Items['filled_evidence_bag'], 'add')
...
RegisterNetEvent('evidence:server:AddCasingToInventory', function(casingId, casingInfo)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if exports.ox_inventory:RemoveItem(src, 'empty_evidence_bag', 1, false, 'evidence:server:AddCasingToInventory') then
        if exports.ox_inventory:AddItem(src, 'filled_evidence_bag', 1, false, casingInfo, 'evidence:server:AddCasingToInventory') then
            TriggerClientEvent('ox_inventory:client:ItemBox', src, QBCore.Shared.Items['filled_evidence_bag'], 'add')
