...
function MenuGarage()
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            ...
            txt = '',
            params = {
                event = 'cafe:client:TakeOutVehicle',
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('cafe:client:TakeOutVehicle', function(data)
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
    if PlayerJob.name == 'cafe' then
        onDuty = PlayerJob.onduty
        if PlayerJob.onduty then
            TriggerServerEvent('cafe:server:AddCafe', PlayerJob.name)
        else
            TriggerServerEvent('cafe:server:RemoveCafe', PlayerJob.name)
        end
    end
end)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    CreateThread(function()
        ...
        QBCore.Functions.GetPlayerData(function(PlayerData)
            ...
            -- end
            if PlayerJob.name == 'cafe' and onDuty then
                TriggerServerEvent('cafe:server:AddCafe', PlayerJob.name)
            end
        end)
    end)
...
RegisterNetEvent('QBCore:Client:SetDuty', function(duty)
    if PlayerJob.name == 'cafe' and duty ~= onDuty then
        if duty then
            TriggerServerEvent('cafe:server:AddCafe', PlayerJob.name)
        else
            TriggerServerEvent('cafe:server:RemoveCafe', PlayerJob.name)
...
local check = false
local function CafeControls(variable)
    CreateThread(function()
        check = true
        while check do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == 'sign' then
                    TriggerEvent('CafeToggle:Duty')
                elseif variable == 'stash' then
                    TriggerServerEvent('qb-cafejob:server:stash')
...
RegisterNetEvent('CafeToggle:Duty', function()
    onDuty = not onDuty
...
CreateThread(function()
    for i = 1, #Config.Locations['vehicle'] do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'cafe' and onDuty then
...
if Config.UseTarget then
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == 'cafe' then
                if not onDuty then
